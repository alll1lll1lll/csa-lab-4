...
class Cache:
    ...
    def read(self, address: int, memory) -> tuple[int, int]:
        """Return (data, ticks). Fills line from memory on miss."""
        if address >= MMIO_BASE:
            return memory.read(address), HIT_TICKS
    ...
    def stats(self) -> str:
        ...
        return (f"Cache stats: {self.hits} hits, {self.misses} misses, "
                f"{total} total, hit rate {ratio:.1f}%")
