...
| :--- | :--- | :--- | :--- |
| **0x00** | `MMIO_IN_STATUS` | read-only | `1` если в очереди ввода есть данные, иначе `0`. |
| **0x04** | `MMIO_IN_DATA` | read-only | Peek первого байта из FIFO-очереди ввода (не извлекает из очереди). |
...
| **0x10** | `MMIO_IRQ_ACK` | write-only | Запись ненулевого значения извлекает первый байт из очереди (pop) и сбрасывает `irq_pending`, если очередь опустела. |
...
В начале каждого такта, указанного в расписании, устройство передаёт байт процессору:
1.  Байт помещается во внутреннюю FIFO-очередь порта ввода (`input_queue`).
2.  Выставляется внешний сигнал запроса на прерывание `irq_pending=1`.

Порт ввода реализует аппаратную буферизацию через FIFO-очередь: если процессор не успел обработать предыдущий байт до прихода следующего, новый байт становится в очередь и не теряется. Чтение `MMIO_IN_DATA` возвращает первый байт из головы очереди (peek). Запись в `MMIO_IRQ_ACK` извлекает его из очереди (pop); если в очереди остались данные — `irq_pending` остаётся в `1`, и обработчик вызывается повторно автоматически.
...
1.  **Запрос прерывания.** Внешнее устройство помещает байт в FIFO-очередь `input_queue` и устанавливает `irq_pending = 1`.
...
5.  **Выполнение обработчика.** Процессор начинает исполнять код обработчика прерываний. Этот код, написанный на ассемблере, должен:
    *   Прочитать байт из порта `MMIO_IN_DATA` (peek из головы FIFO).
    *   Подтвердить получение, записав ненулевое значение в `MMIO_IRQ_ACK` — это извлечёт байт из очереди (pop). Если в очереди ещё есть данные, `irq_pending` остаётся в `1` и после `iret` будет вызван следующий вход в обработчик.
