...
class InstructionEncoder:
    ...
    def encode_r(opcode, rd, rs1, rs2, funct3, funct7):
        ...
        return ((opcode & 0x3F) << 26) | ((rd & 0x1F) << 21) | \
               ((rs1 & 0x1F) << 16) | ((rs2 & 0x1F) << 11) | \
               ((funct3 & 0x7) << 8) | (funct7 & 0xFF)

    @staticmethod
    def encode_i(opcode, rd, rs1, imm, funct3):
        # [opcode:6][rd:5][rs1:5][funct3:3][imm:13]
        return ((opcode & 0x3F) << 26) | ((rd & 0x1F) << 21) | \
               ((rs1 & 0x1F) << 16) | ((funct3 & 0x7) << 13) | (imm & 0x1FFF)

    @staticmethod
    def encode_s(opcode, rs1, rs2, imm, funct3):
        # [opcode:6][imm[12:9]:4][rs1:5][rs2:5][funct3:3][imm[8:0]:9]
        imm = imm & 0x1FFF
        return ((opcode & 0x3F) << 26) | (((imm >> 9) & 0xF) << 22) | \
               ((rs1 & 0x1F) << 17) | ((rs2 & 0x1F) << 12) | \
               ((funct3 & 0x7) << 9) | (imm & 0x1FF)
